...
//
// This file carries no build tag, unlike the four commands behind
// secret.go's 'secret' tag: it runs on every machine that can check out
// the secrets repository, because declaring what a credential needs is an
// authoring act, not a custody one. Its own init() registers the 'secret'
// namespace itself — see secret_common.go for why exactly one file may do
// that — and secret.go's init() extends the same table with 'create',
// 'rekey', 'migrate', and 'rotate' from behind its tag.
//
// The three writes themselves are textual insertions found by pattern, not
// generated by evaluating the secrets flake; every insertion is one
// contiguous block, and every other byte of every file is untouched.
// declare does need 'nix' on PATH for two things it cannot get from text
// alone: deriving each --to machine's target kind from the inventory
// flake's own 'machines.<name>.type' rather than taking it as a flag, so
// nothing on the command line can misdeclare a machine's own type; and
// checking the secrets checkout's own evaluated credential inventory
// (lib.credentials) for the name, because credentials.nix generates some
// entries from attrsets a text scan cannot see into (see
// declareEvalCredentialNames). Both are read-only evaluations; nothing
// declare writes depends on how the branch it edits evaluates, which is
// why a 'pending' entry with no ciphertext — exactly the shape
// credential-inventory accepts — still lands cleanly.
...
func init() {
	registerNamespace(namespace{
		name:    "secret",
		summary: "Write or land a credential; declare works everywhere, the landing commands need the secret tag",
		main:    secretMain,
	})
...
const declareUsageLine = `allod secret declare <name> [<checkout>] --kind <kind> --owner <owner> --to <machine>[,<machine>...] --deployed-path <path> --verify <command> [--value-template-file <path>|--value-template-stdin] [--value-encode <name>] [--service none|forgejo] [--account <account>] [--ui-token-name <name>] [--strategy overlap|in-place]`

// declareValidKinds excludes credential-inventory's other two kinds,
// forge-git and machine-host: the shape declare writes (public_key null,
// one agenix consumer) is exactly what the inventory check rejects for
// them — a forge-git entry needs a real public_key and one
// forge-key-secret plus one forgejo-ssh consumer, and machine-host entries
// are generated by hostEntries, never declared by hand.
var declareValidKinds = []string{"user", "agent", "service"}
...
<name> must match ^[a-z][a-z0-9-]*$ and must not be a nix keyword (let, in,
with, rec, assert, if, then, else, inherit, or) — it becomes an unquoted
nix attribute name, a secrets.nix key, and a filename, and none of those
parse as one. --kind is the credential's own kind and must be one of:
user, agent, service — credential-inventory's other two kinds, forge-git
and machine-host, are refused: the shape declare writes (public_key null,
one agenix consumer) is exactly what the inventory check rejects for
them, since forge-git needs a real public_key and a different consumer
pair and machine-host entries are generated by the template, never
hand-declared. --kind, --owner, --account, and --ui-token-name must also
match ^[A-Za-z0-9][A-Za-z0-9_.@-]*$ — these values reach generated nix or
JSON text, so their shape is checked before any of it is built, not
interpolated raw. --to names one or
...
types. --deployed-path must be an absolute path with no whitespace, control
character, quote, backslash, or $, and applies to every target.

The credential's value is the secret itself unless a template is given.
--value-template-file <path> or --value-template-stdin (the two are
mutually exclusive) reads the non-secret text around the secret; the bytes
are stored exactly as read, with no trimming, so whether the template ends
in a newline is part of the declaration. A template must contain exactly
one {secret} placeholder, which is where 'create' and 'rotate' put the
value. --value-encode <name> names an encoding applied to the secret alone
before it is substituted; it requires a template and must be a name the
checkout's own lib.credentialEncodings exports.

--verify is one command line, non-empty and without a line break, carried
on every target. 'allod secret rotate' prints it verbatim for a target on
this machine and behind ssh for any other, so it is the command as it runs
on the target: 'sudo env HOME=/root GIT_TERMINAL_PROMPT=0 git ls-remote
<url> HEAD', 'sudo -u <user> forge token verify < <path>', 'allod site
check', and 'tailscale status --peers=false' are the four rotate-token used
to build from a probe type, now written out once here.

--service defaults to "none"; "forgejo" requires --account and
--ui-token-name, and "none" refuses them. --strategy defaults to
"overlap".

declare refuses a name already declared anywhere: a credentials.nix entry
(matched even split across lines or separated by a nix comment from its
'= {'), a secrets.nix line for secrets/<name>.age (matched the same way),
a registry group keyed <name> or naming <name> among its credentials, or
an entry the secrets checkout's own evaluated credential inventory
(<checkout>#lib.credentials) already carries — credentials.nix is
'hostEntries // activeEntries // stagedEntries // forgeGitEntries // {
...literal entries...; }', so a name one of those generated attrsets
already produces is invisible to every textual check above, and the
literal entry declare would append there would silently shadow it. A
failed evaluation refuses outright rather than relying on the textual
checks alone. Every match found is listed. It
...
type declareArgs struct {
	...
	owner              string
	to                 []string
	valueTemplateFile  string
	valueTemplateStdin bool
	valueEncode        string
	deployedPath       string
	verify             string
	service            string
...
// declareNixKeywords are the identifiers nix reserves, every one of which
// matches declareNamePattern and would still fail to parse as an unquoted
// attribute name ('let = { ... };' is a syntax error, not an attribute
// named "let").
var declareNixKeywords = []string{
	"let", "in", "with", "rec", "assert", "if", "then", "else", "inherit", "or",
}
...
func parseDeclareArgs(args []string) declareArgs {
	...
	for len(args) > 0 {
		...
		case "-h", "--help":
			...
			exit(0)
		case "--kind", "--owner", "--value-template-file", "--value-encode", "--deployed-path", "--verify", "--service", "--account", "--ui-token-name", "--strategy":
			if len(args) < 2 {
			...
			case "--owner":
				parsed.owner = value
			case "--value-template-file":
				parsed.valueTemplateFile = value
			case "--value-encode":
				parsed.valueEncode = value
			case "--deployed-path":
			...
			args = args[2:]
		case "--value-template-stdin":
			parsed.valueTemplateStdin = true
			args = args[1:]
		case "--to":
			if len(args) < 2 {
	...
	if stringInList(parsed.name, declareNixKeywords) {
		secretCommandUsageError("declare", "invalid credential name %q: a nix keyword cannot be an unquoted attribute name", parsed.name)
	}
	if parsed.kind == "" {
	...
	if !stringInList(parsed.kind, declareValidKinds) {
		secretCommandUsageError("declare", "invalid --kind %q; expected one of: %s (forge-git and machine-host entries are generated by the template, not declared)", parsed.kind, joinList(declareValidKinds))
	}
	if parsed.owner == "" {
	...
	if parsed.valueTemplateFile != "" && parsed.valueTemplateStdin {
		secretCommandUsageError("declare", "--value-template-file and --value-template-stdin are mutually exclusive")
	}
	if parsed.valueEncode != "" && parsed.valueTemplateFile == "" && !parsed.valueTemplateStdin {
		secretCommandUsageError("declare", "--value-encode requires a value template")
	}
	...
	if strings.TrimSpace(parsed.verify) == "" || strings.ContainsAny(parsed.verify, "\r\n") {
		secretCommandUsageError("declare", "--verify must be one non-empty command line")
	}
...
// --- The evaluated credential inventory ---

// declareEvalCredentialNames evaluates the secrets checkout's own
// credential inventory (lib.credentials — see README.md and flake.nix in
// the secrets repository) and returns every name it already declares.
// credentials.nix is 'hostEntries // activeEntries // stagedEntries //
// forgeGitEntries // { ...literal entries...; }': a name produced by one
// of those generated attrsets is invisible to the textual collision
// checks, which only see the literal block, and the entry declare appends
// there would silently shadow it (the last '//' operand wins in nix). This
// check sees every name the flake actually evaluates, generated or
// literal; the textual checks stay too, since they also catch a name on a
// branch that does not evaluate cleanly yet — the state a 'pending' entry
// is deliberately left in.
func declareEvalCredentialNames(checkout string) (map[string]struct{}, error) {
...
// declareTarget's Verify is the one command line the target carries,
// written out on every target of the declared credential. There is no
// probe type and no command-specific field beside it: whatever the command
// needs — a repository URL, a user, a path — is part of the string.
...
type declareCredentialEntry struct {
	...
	Value      *declareValue   `json:"value,omitempty"`
...
func buildDeclareGroup(parsed declareArgs, targets []declareTarget, value *declareValue) declareGroup {
	group := declareGroup{
...
// declareNixGap is what may sit between two tokens of an existing
// declaration without hiding it from the collision regexes below:
// whitespace (including newlines, so a declaration split across lines is
// still found) interleaved with nix line comments ('#' to end of line),
// since a comment between the name and '=' hides nothing from nix either.
const declareNixGap = `(?:\s|#[^` + "\n" + `]*)*`

// declareCredentialsHasEntry reports whether credentials.nix already
// declares a literal '<name> = { ... }' or '"<name>" = { ... }' attribute,
// the same shape flipPendingToActive (secret.go) looks for. A one-line,
// no-comment match would let a duplicate split across lines, or separated
// by a comment, through the gate.
func declareCredentialsHasEntry(text, name string) bool {
	pattern := regexp.MustCompile(`(?m)^[ \t]*"?` + regexp.QuoteMeta(name) + `"?` + declareNixGap + `=` + declareNixGap + `\{`)
	return pattern.MatchString(text)
}

// declareSecretsHasLine reports whether secrets.nix already declares a
// publicKeys line for the given repository-relative path, for the same
// reason and against the same gap declareCredentialsHasEntry allows.
func declareSecretsHasLine(text, path string) bool {
	pattern := regexp.MustCompile(`(?m)^[ \t]*"` + regexp.QuoteMeta(path) + `"` + declareNixGap + `\.publicKeys` + declareNixGap + `=`)
	return pattern.MatchString(text)
}
...
func declareCollisions(credentialsText, secretsText string, registry map[string]registryGroup, evaluatedCredentials map[string]struct{}, name, secretPath string) []string {
	...
	if _, exists := evaluatedCredentials[name]; exists {
		findings = append(findings, fmt.Sprintf("lib.credentials (evaluated): an entry named '%s' already exists", name))
	}
...
func declareInsertRegistryGroup(text, name string, group declareGroup) (string, error) {
	...
	// HTML escaping is off: a verify command routinely redirects a file
	// into it ('forge token verify < <path>'), and '<' in the registry
	// would be correct JSON that no reviewer can read.
	...
	groupJSON := bytes.TrimRight(encoded.Bytes(), "\n")
...
// and renames it over path, so path is always either its previous bytes in
// full or its new bytes in full — never a partial write, which a direct
// os.WriteFile leaves on disk if it fails partway through. It lives here,
// untagged, because 'declare' needs it, and it is the one writer every
// other command in the namespace uses too: 'create', 'rekey', 'rotate', and
// 'migrate' all replace files whose half-written state would be a
// ciphertext no machine can decrypt or a registry no flake can parse.
func atomicWrite(path string, data []byte) error {
	tmp, err := os.CreateTemp(filepath.Dir(path), ".allod-"+filepath.Base(path)+"-*.tmp")
	if err != nil {
		return err
	}
	tmpPath := tmp.Name()
...
func declareRestore(done []declareWrite) string {
	...
	for _, write := range done {
		if err := atomicWrite(write.path, write.original); err != nil {
...
func declareWriteAll(writes []declareWrite) {
	...
	for _, write := range writes {
		...
		if err := atomicWrite(write.path, write.content); err != nil {
...
func secretDeclare(args []string) {
	...
	// The template is stored exactly as read: no trim, no newline added or
	// removed, because whether it ends in one is part of the credential's
	// bytes on every machine that deploys it. That promise is only
	// keepable for UTF-8: the template travels as JSON text, and
	// encoding/json replaces an invalid byte with U+FFFD rather than
	// failing, so a template that is not UTF-8 would be stored silently
	// altered and deployed as bytes nobody wrote. It is refused instead.
	var value *declareValue
	if parsed.valueTemplateFile != "" {
		data, err := os.ReadFile(parsed.valueTemplateFile)
		if err != nil {
			die(1, "could not read --value-template-file %s: %s", parsed.valueTemplateFile, err)
		}
		requireUTF8Template(data, "--value-template-file "+parsed.valueTemplateFile)
		value = &declareValue{Template: string(data), Encode: parsed.valueEncode}
	} else if parsed.valueTemplateStdin {
		data, err := io.ReadAll(stdin)
		if err != nil {
			die(1, "could not read the value template from stdin: %s", err)
		}
		requireUTF8Template(data, "the value template on stdin")
		value = &declareValue{Template: string(data), Encode: parsed.valueEncode}
	}
	if value != nil {
		if strings.Count(value.Template, "{secret}") != 1 {
			secretCommandUsageError("declare", "value template must contain exactly one {secret} placeholder")
		}
		if value.Encode != "" {
			// The list lives in the checkout, not in this command: adding
			// an encoding is a change to the secrets flake, and declare
			// reads whatever that flake exports today.
			...
			if err := json.Unmarshal(data, &encodings); err != nil {
				die(1, "lib.credentialEncodings in %s is not a list of strings", checkout)
			}
			if !stringInList(value.Encode, encodings) {
				secretCommandUsageError("declare", "--value-encode %q is not exported by lib.credentialEncodings", value.Encode)
			}
		}
	}
	...
	// A failed evaluation refuses outright rather than falling through to
	// the text checks alone: those only see the literal block at the end
	// of credentials.nix, not a name credentials.nix generates.
	evaluatedCredentials, err := declareEvalCredentialNames(checkout)
	if err != nil {
		die(1, "could not evaluate %s#lib.credentials: %s; refusing rather than relying on the text checks alone", checkout, err)
	}

	secretPath := "secrets/" + parsed.name + ".age"
	if findings := declareCollisions(credentialsText, secretsText, registry, evaluatedCredentials, parsed.name, secretPath); len(findings) > 0 {
...
// requireUTF8Template refuses template bytes that are not valid UTF-8.
// source names where the bytes came from, so the refusal points at the file
// or the pipe rather than at the registry it would have landed in.
func requireUTF8Template(data []byte, source string) {
	if !utf8.Valid(data) {
		secretCommandUsageError("declare", "%s is not valid UTF-8; a value template is stored as JSON text, which carries UTF-8 and nothing else, and every byte of a template is preserved as given", source)
	}
}
