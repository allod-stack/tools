...
const declareUsageLine = `allod secret declare <name> [<checkout>] --kind <kind> --owner <owner> --to <machine>[,<machine>...] --format <format> --deployed-path <path> --verify <probe> [--verify-repo-url <url>] [--verify-context <text>] [--verify-user <user>] [--service none|forgejo] [--account <account>] [--ui-token-name <name>] [--strategy overlap|in-place]`
...
backslash, or $, and applies to every target. --verify is one of:
forge-token-verify, git-ls-remote, site-check, and rotate-token's own
printed verification instructions (print_group_verification,
scripts/rotate-token in nexus) read further fields no target carries by
default: --verify-repo-url <https:// url, required with git-ls-remote,
refused otherwise> and --verify-context <label, optional, git-ls-remote
only> for the 'git ls-remote <url> HEAD' it prints, and --verify-user
<required with forge-token-verify, refused otherwise> for the 'sudo -u
<user> forge token verify' it prints on a nixos-host target. --service
defaults to "none"; "forgejo"
...
// declareControlChar is what --verify-context refuses: it is print-only
// (rotate-token only ever passes it to printf as a %s argument, never
// interpolates it into a command), so its shape only needs to stay
// printable on one line.
var declareControlChar = regexp.MustCompile(`[\x00-\x1f\x7f]`)

func validateVerifyContext(text string) {
	if text == "" || declareControlChar.MatchString(text) {
		secretCommandUsageError("declare", "invalid --verify-context %q; expected non-empty printable text with no control characters", text)
	}
}
...
func parseDeclareArgs(args []string) declareArgs {
	...
	for len(args) > 0 {
		...
		case "-h", "--help":
			...
			exit(0)
		case "--kind", "--owner", "--format", "--deployed-path", "--verify", "--verify-repo-url", "--verify-context", "--verify-user", "--service", "--account", "--ui-token-name", "--strategy":
			if len(args) < 2 {
			...
			case "--verify":
				parsed.verify = value
			case "--verify-repo-url":
				parsed.verifyRepoURL = value
			case "--verify-context":
				parsed.verifyContext = value
			case "--verify-user":
				parsed.verifyUser = value
			case "--service":
	...
	// rotate-token's printed verification (print_group_verification,
	// scripts/rotate-token in nexus) reads .verify.repo_url and
	// .verify.credential_context for a git-ls-remote target and prints '-'
	// when either is absent, which is not a command 'git ls-remote - HEAD'
	// can run. --verify-repo-url is load-bearing for that command and is
	// required; --verify-context only labels it ('%s (%s):') and stays
	// optional. The same script's forge-token-verify branch runs 'sudo -u
	// <user> forge token verify' for a nixos-host target, so --verify-user
	// is required there for the same reason. Neither pair means anything
	// for the other probe types, so both are refused outside their own.
	switch parsed.verify {
	case "git-ls-remote":
		if parsed.verifyRepoURL == "" {
			secretCommandUsageError("declare", "--verify git-ls-remote requires --verify-repo-url")
		}
		validateVerifyRepoURL(parsed.verifyRepoURL)
		if parsed.verifyContext != "" {
			validateVerifyContext(parsed.verifyContext)
		}
	default:
		if parsed.verifyRepoURL != "" || parsed.verifyContext != "" {
			secretCommandUsageError("declare", "--verify-repo-url and --verify-context apply only to --verify git-ls-remote")
		}
	}
	switch parsed.verify {
	case "forge-token-verify":
		if parsed.verifyUser == "" {
			secretCommandUsageError("declare", "--verify forge-token-verify requires --verify-user")
		}
		validateIdentifier("--verify-user", parsed.verifyUser)
	default:
		if parsed.verifyUser != "" {
			secretCommandUsageError("declare", "--verify-user applies only to --verify forge-token-verify")
		}
	}
...
// declareVerify's RepoURL and CredentialContext are only ever set for the
// git-ls-remote probe (parseDeclareArgs refuses them otherwise); omitted
// for every other type, matching the shape existing groups in the template
// use for forge-token-verify and site-check.
...
// declareTarget's User is only ever set for the forge-token-verify probe
// (parseDeclareArgs refuses it otherwise); omitted for every other type.
...
func secretDeclare(args []string) {
	...
	for _, machine := range parsed.to {
		targets = append(targets, declareTarget{
			...
			User:         parsed.verifyUser,
			Verify: declareVerify{
				Type:              parsed.verify,
				RepoURL:           parsed.verifyRepoURL,
				CredentialContext: parsed.verifyContext,
			},
		})
	}
