...
`<destination-repo>` and `apply --repo`'s value each accept either a filesystem path or a repository-registry id such as `allod/memory`: a repo argument that is absolute or begins with `~`, `.`, or `..` is a path, and anything else is looked up in the repository registry — `$INVENTORY/scripts/repositories.json` when `INVENTORY` is set, `allod/inventory/scripts/repositories.json` under the work directory (`~/work`, or `WORK_DIR` when set) otherwise — first and, when no entry matches, treated as a relative path.
