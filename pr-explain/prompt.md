...
Write **body-fragment-only output** to the exact path in `ALLOD_PR_EXPLAIN_REPORT_BODY`. The tool pre-creates that private regular file: populate it in place (for example, with shell redirection), and do not unlink, rename, replace, or symlink it. Do not print the report to standard output. The fragment begins with the skip link and ends with the footer. Do not emit a doctype, `html`, `head`, `body`, `title`, `style`, or `script` element. The tool owns the document shell and injects canonical CSS and JavaScript byte-for-byte.
